...
- Analysis 섹션은 아래 3단 구조를 따른다 (상세: Analysis 섹션 가이드)
...
Analysis는 개별 기사 요약이 아니라, **1층(인식 확장) → 2층(전략 판단)**을 연결하는 핵심 섹션이다.

### 3단 구조

각 Analysis 항목(×3)은 반드시 아래 세 단락으로 구성한다:

1. **Signal** — 이번 주 기사에서 포착된 구조적 변화 신호
   - 팩트 중심. 어떤 일이, 어디서, 어떤 규모로 벌어지고 있는가.
2. **Pattern** — 왜 지금 이 변화가 발생했는가
   - 3국 비교, 시계열 변화, 또는 산업 구조적 맥락을 제시.
   - 단순 해석이 아닌, 반복되는 패턴이나 구조적 전환을 드러낸다.
3. **Implication** — 한국 엔터테인먼트 사업자에게 의미하는 것
   - 구체적 기업명·업종·상황을 예시로 포함 (CJ, HYBE, 카카오엔터, 넷마블 등)
   - "~할 수 있다", "~를 주시해야 한다" 등 액션 지향 문장으로 마무리.
...
- **1회 독자**: Analysis 마지막 단락(Implication)만 읽어도 "이게 나한테 왜 중요한지" 파악 가능
- **4회(1개월) 독자**: Analysis 3개 × 4주 = 12개의 Signal-Pattern을 모으면 지역별 트렌드 맵 완성
- **12회(3개월) 독자**: Implication의 누적이 사업 전략 방향의 근거 자료가 됨
